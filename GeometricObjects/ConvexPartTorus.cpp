...
ConvexPartTorus::ConvexPartTorus (void)
	: 	GeometricObject(),
		...
		b(0.5),
		min_azimuth(0),
		max_azimuth(TWO_PI<double>),
		min_polar(0),
		max_polar(TWO_PI<double>),
...
ConvexPartTorus::ConvexPartTorus (const double _a, const double _b, const double _min_azimuth, const double _max_azimuth, const double _min_polar, const double _max_polar)
	: 	GeometricObject(),
		...
		b(_b),
		bbox(-a - b, a + b, -b, b, -a - b, a + b),
		min_azimuth(_min_azimuth*PI_ON_180<double>),
		max_azimuth(_max_azimuth*PI_ON_180<double>),
		min_polar(_min_polar*PI_ON_180<double>),
		max_polar(_max_polar*PI_ON_180<double>)
{}																			
...
ConvexPartTorus::hit(const Ray& ray, double& tmin, ShadeRec& sr) const {
	...
	for(int j = 0; j < num_real_roots; ++j)
	{
		if (roots[j] > kEpsilon )
		{
			...
			{
				Point3D point(a * cos(-phi+PI<double>/2),0,a * sin(-phi+PI<double>/2));
...
bool 																						 
ConvexPartTorus::shadow_hit(const Ray& ray, double & tmin) const
{
	/*
	if (!bbox.hit(ray))
		return (false);

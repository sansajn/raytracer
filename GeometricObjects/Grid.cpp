...
void Grid::read_flat_uv_triangles(char const * file_name) {
...
void Grid::read_smooth_triangles(char const * file_name) {
  	read_ply_file(file_name, smooth);
  	compute_mesh_normals();
}

void Grid::read_smooth_uv_triangles(char const * file_name) {
...
void Grid::read_uv_ply_file(char const * file_name, const int triangle_type) {
	// Vertex definition
...
bool Grid::shadow_hit(Ray const & ray, double & t) const {
	double ox = ray.o.x;
	double oy = ray.o.y;
	double oz = ray.o.z;
	double dx = ray.d.x;

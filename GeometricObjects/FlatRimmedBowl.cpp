...
FlatRimmedBowl::build_components(void) {
	...
	add_object(new Annulus(Point3D(0.0),Normal(0,1,0),inner_radius,outer_radius-inner_radius));   			// maximum polar angle measured from top
}

void
FlatRimmedBowl::set_material(shared_ptr<Material> m_ptr) {
	for (int j = 0; j < 3; j++)

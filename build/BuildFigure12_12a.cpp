...
void World::build() {
	...
	auto left_camera_ptr = make_unique<Pinhole>();
	left_camera_ptr->set_view_distance(vpd);
	
	auto right_camera_ptr = make_unique<Pinhole>();
	right_camera_ptr->set_view_distance(vpd);
	
	auto stereo_ptr = make_unique<StereoCamera>(move(left_camera_ptr), move(right_camera_ptr));
...
//	stereo_ptr->use_transverse_viewing();
	...
	phong_ptr1->set_cd(RGBColor{0, 0.5, 0.5});   	// cyan
	...
	phong_ptr2->set_cd(RGBColor{0.85, 0.6, 0.2}); 	// brown
	...
	phong_ptr3->set_cd(RGBColor{1, 1, 0});     		// yellow

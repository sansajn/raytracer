...
//	Sampler* sampler_ptr = new MultiJittered(num_samples);
	...
	auto disk_ptr = make_unique<Disk>(center, normal, radius);
	disk_ptr->set_material(make_shared<Emissive>(white, 40));
	disk_ptr->set_sampler(/*sampler_ptr*/make_shared<MultiJittered>(num_samples));
	disk_ptr->set_shadows(false);
//	disk_ptr->compute_uvw();
//	add_object(disk_ptr);
	
	AreaLight* area_light_ptr = new AreaLight;
	area_light_ptr->set_object(move(disk_ptr));

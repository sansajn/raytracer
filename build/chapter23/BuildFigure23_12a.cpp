...
//	bunny_ptr->set_material(phong_ptr);				// only use this if you want white bunnies
	...
	auto current_grid_ptr = bunny_ptr;							// just the bunny
	
	for (int level = 0; level < num_levels; level++) {	
		auto instance_grid_ptr = make_shared<Grid>();						// temporary grid
	...
	Grid * grid = current_grid_ptr->clone();
	add_object(grid); 								// the whole n-level grid

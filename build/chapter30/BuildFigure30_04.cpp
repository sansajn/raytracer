...
// In this scene the line widths are all zero so that the checkers are not outlined.
// The Chapter 30 download has a version of this scene where the checkers rendered with colored outlines. 
...
#include "Textures/CylinderChecker.h"
#include "Textures/RectangleChecker.h"
...
void World::build() {
	...
	Instance* sphere_ptr = new Instance(make_shared<Sphere>());
	...
	Instance* cone_ptr = new Instance(make_shared<OpenCone>());
	cone_ptr->set_material(sv_matte_ptr3);
	cone_ptr->scale(Vector3D{2.5});
	...
	Instance* front_face_ptr = new Instance(make_shared<Rectangle>());
	...
	Instance* top_face_ptr = new Instance(make_shared<Rectangle>());
	...
	auto sv_matte_ptr7 = make_shared<SV_Matte>();
	sv_matte_ptr7->set_ka(0.75);
	sv_matte_ptr7->set_kd(0.75);
	sv_matte_ptr7->set_cd(rectangle_checker_ptr3);
	
	Instance* right_face_ptr = new Instance(make_shared<Rectangle>());
	...
	auto generic_box_ptr = make_shared<Compound>();
	...
	plane_checker_ptr->set_color1(RGBColor{.35});
	plane_checker_ptr->set_color2(gray);
	plane_checker_ptr->set_outline_color(yellow);
		
	auto sv_matte_ptr8 = make_shared<SV_Matte>();
	sv_matte_ptr8->set_ka(0.75);
	sv_matte_ptr8->set_kd(0.75);
	sv_matte_ptr8->set_cd(plane_checker_ptr);
	
	Instance* plane_ptr = new Instance(make_shared<Plane>()); // a generic plane is the (x, z) plane
